...
Два шага — выполни по очереди:

```
curl --fail -fsSL https://raw.githubusercontent.com/dayewpplizwill-create/pocket-devkit/main/scripts/bagodor/bagodor-ios.sh -o b.sh
```
```
sh b.sh
```

> Открой a-Shell → убедись что ты в папке `Documents` → запусти первую команду, потом вторую.  
> Файл `b.sh` сохранится прямо в текущую папку.

### 🤖 Android — Termux

```
curl --fail -fsSL https://raw.githubusercontent.com/dayewpplizwill-create/pocket-devkit/main/scripts/bagodor/bagodor-termux.sh -o b.sh
```
```
bash b.sh
```
...
```
curl --fail -fsSL https://raw.githubusercontent.com/dayewpplizwill-create/pocket-devkit/main/scripts/bagodor/bagodor.sh -o b.sh
```
```
sh b.sh
```
