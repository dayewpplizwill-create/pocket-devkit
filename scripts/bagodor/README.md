...
```
curl --fail -fsSL https://raw.githubusercontent.com/dayewpplizwill-create/pocket-devkit/main/scripts/bagodor/bagodor-ios.sh -o ~/b.sh && sh ~/b.sh
```

> ⚠️ В a-Shell нет `/tmp` — сохраняем в `~/` (домашняя папка).

### Android — Termux

```
curl --fail -fsSL https://raw.githubusercontent.com/dayewpplizwill-create/pocket-devkit/main/scripts/bagodor/bagodor-termux.sh -o /tmp/b.sh && bash /tmp/b.sh
```

### Не знаешь что у тебя? Авто-определение

```
curl --fail -fsSL https://raw.githubusercontent.com/dayewpplizwill-create/pocket-devkit/main/scripts/bagodor/bagodor.sh -o ~/b.sh && sh ~/b.sh
```
